...
const CopyForm: React.FC<CopyFormProps> = ({ numPages, setNumPages, isColor, setIsColor, isBound, setIsBound, handleCalculate, totalCost }) => (
  <div className="w-full max-w-md px-4 flex flex-col items-center">
    ...
    <Button
      ...
      className="w-full max-w-xs bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mt-4">
      Calcular
    </Button>

    <div className="w-full max-w-xs mt-4 bg-white p-4 rounded shadow-md text-center">
      <h2 className="text-xl font-bold text-orange-600">Custo Total: R$ {totalCost.toFixed(2) == "NaN" ? "0.00" : totalCost.toFixed(2)}</h2>
    </div>
  </div>
);
